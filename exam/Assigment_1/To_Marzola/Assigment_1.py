            ...
            if NM_ASGD and epoch % Logging_interval == 0 and T == 0:
                if (len(loss_log) > 0):
                    if t > non_monotone_interval and ppl_dev > min(loss_log):
                        T = epoch
                        print("Parametri",modelM.parameters())
                        optimizer= ASGD_optim(modelM.parameters(), lr=lr,
                                  weight_decay=weight_decay_Av)
                        
                        print(f"swiching to ASGD at epoch{epoch}")
                        # Since there is the passage to the new optinizer a bit of patiance more is better
                        patience = patience_set + 2
...
Lr = [1.0, 2.0, 3.0]
Clip = [1, 5, 10]
Hid_size = [200, 300, 500]
Emb_size = [200, 300, 500]
